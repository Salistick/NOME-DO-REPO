...
class YouTubeAuth:
    ...
    def update_account_token(self, account_data: dict) -> None:
        self._upsert_account_token(account_data)
    ...
    def remove_account_by_account_id(self, account_id: str) -> bool:
        return self._remove_account_token_by_account_id(account_id)
    ...
    def run_browser_login(self, cancel_event: threading.Event | None = None) -> dict:
        ...
        try:
            ...
            account_payload = {
                ...
                "scopes": self._parse_scope_value(token_data.get("scope", "")),
                "channels": channels,
            }
        ...
        if refresh_token:
            try:
                ...
                if refreshed.get("scope"):
                    account["scope"] = refreshed.get("scope", "")
                    account["scopes"] = self._parse_scope_value(refreshed.get("scope", ""))
    ...
    def get_valid_account_by_index(self, index: int) -> dict:
        ...
        return self._ensure_valid_cached_account(account)

    def get_valid_account_by_account_id(self, account_id: str) -> dict:
        account_id = (account_id or "").strip()
        if not account_id:
            raise RuntimeError("account_id do YouTube nao informado.")

        for account in self.list_cached_accounts():
            if (account.get("account_id") or "").strip() == account_id:
                return self._ensure_valid_cached_account(account)

        raise RuntimeError("Conta YouTube selecionada nao encontrada no cache.")

    def _ensure_valid_cached_account(self, account: dict) -> dict:
    ...
    def account_has_chat_send_scope(self, account: dict | None) -> bool:
        if not account:
            return False

        scopes = account.get("scopes")
        if isinstance(scopes, list):
            scope_set = {str(item or "").strip() for item in scopes}
        else:
            scope_set = set()

        raw_scope = str(account.get("scope") or "").strip()
        if raw_scope:
            scope_set.update(self._parse_scope_value(raw_scope))

        return bool(
            "https://www.googleapis.com/auth/youtube.force-ssl" in scope_set
            or "https://www.googleapis.com/auth/youtube" in scope_set
        )
