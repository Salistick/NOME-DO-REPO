...
class YouTubeChatMonitor:
    ...
    def _parse_item(self, item) -> Optional[dict]:
        try:
            ...
            if author is not None:
                author_name = self._get_author_value(author, "name") or ""
                author_channel_id = self._get_author_value(author, "channelId") or ""

            message_text = getattr(item, "message", "") or ""
            author_role = self._detect_author_role(author)
            ...
            return {
                ...
                "is_mod": author_role["is_mod"],
                "is_sub": author_role["is_sub"],
                "is_broadcaster": author_role["is_broadcaster"],
            }
    ...
    def _detect_author_role(self, author) -> dict:
        author_type = str(self._get_author_value(author, "type") or "").strip().upper()

        is_broadcaster = (
            self._truthy_author_value(author, "isChatOwner")
            or author_type == "OWNER"
        )
        is_mod = (
            self._truthy_author_value(author, "isChatModerator")
            or author_type == "MODERATOR"
        )
        is_sub = (
            self._truthy_author_value(author, "isChatSponsor")
            or author_type == "MEMBER"
        )
    ...
    def _get_author_value(self, author, key: str):
        if author is None:
            return ""
        ...
        return getattr(author, key, "")

    def _truthy_author_value(self, author, key: str) -> bool:
        value = self._get_author_value(author, key)
